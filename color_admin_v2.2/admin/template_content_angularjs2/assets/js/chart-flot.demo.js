...
var handleStackedChart = function () {
    ...
    $.plot("#flot-stacked-chart", xData, options);
    ...
    $("#flot-stacked-chart").bind("plothover", function (event, pos, item) {
        if (item) {
...
var handleTrackingChart = function () {
	...
	if ($('#flot-tracking-chart').length !== 0) {
        var plot = $.plot($("#flot-tracking-chart"),
        [ 
        ...
        var legends = $("#flot-tracking-chart .legendLabel");
        ...
        $("#flot-tracking-chart").bind("plothover",  function (pos) {
            latestPosition = pos;
...
var handleBarChart = function () {
	...
	if ($('#flot-bar-chart').length !== 0) {
        ...
        $.plot("#flot-bar-chart", [ {data: data, color: purple} ], {
            series: {
                bars: {
                    show: true,
                    barWidth: 0.4,
...
var handleInteractivePieChart = function () {
	"use strict";
	if ($('#flot-interactive-pie-chart').length !== 0) {
        var data = [];
        var series = 3;
        ...
        $.plot($("#flot-interactive-pie-chart"), data,
        {
            series: {
                pie: { 
                    show: true
...
var handleInteractiveChart = function () {
	...
	if ($('#flot-interactive-chart').length !== 0) {
        ...
        $("#flot-interactive-chart").bind("plothover", function (event, pos, item) {
...
var handleLiveUpdatedChart = function () {
	...
	if ($('#flot-live-updated-chart').length !== 0) {
        var data = [], totalPoints = 150;
        
        // setup control widget
        var updateInterval = 1000;
        $("#flot-updateInterval").val(updateInterval).change(function () {
            var v = $(this).val();
            if (v && !isNaN(+v)) {
                updateInterval = +v;
                if (updateInterval < 1) {
        ...
        var plot = $.plot($("#flot-live-updated-chart"), [ getRandomData() ], options);
